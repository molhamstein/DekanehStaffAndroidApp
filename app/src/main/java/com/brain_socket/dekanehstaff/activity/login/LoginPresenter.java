...
public class LoginPresenter<T extends LoginVP.View> extends BasePresenterImpl<T> implements LoginVP.Presenter<T> {
    ...
    public void onAttach(T mvpView) {
        ...
        if (getCacheStore().getSession().isLoggedOn()) {
            if (getCacheStore().getSession().getRole().equals(Enums.Roles.warehouseKeeper.toString()))
                getView().startWarehouseMainActivity();
            else
                        ...
                        .subscribe(new Consumer<LoginResponse>() {
                            ...
                            public void accept(LoginResponse loginResponse) throws Exception {
                                if (loginResponse.getUser().getStatus() == User.Status.activated) {
                                    ...
                                    getCacheStore().getSession().setRole(loginResponse.getUser().getRoles().get(0).getNameAr());
                                    if (getCacheStore().getSession().getRole().equals(Enums.Roles.warehouseKeeper.toString()))
                                        getView().startWarehouseMainActivity();
                                    else
                                        getView().startMainActivity();
